...
{
    public class bookspanelModel : PageModel
    {
        public class Item
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@Category", "books");
                        ...
                        using (SqlDataReader reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                ...
                                {
                                    item.Location = reader["FoundLocation"]?.ToString() ?? "";
                                    FoundItems.Add(item);
                                }
                                else if (item.ItemStatus == "lost")
                                {
                                    item.Location = reader["LostLocation"]?.ToString() ?? "";
                                    LostItems.Add(item);
                                }
                            }
                        }
            ...
            {
                System.Console.WriteLine($"Error fetching book items: {ex.Message}");
            }
        }
    }
}
